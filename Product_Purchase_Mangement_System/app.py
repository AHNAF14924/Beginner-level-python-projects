...
def process_product_choice(product_category):
    ...
    print(tabulate(numbered_data, headers=[''] + header, tablefmt='grid', showindex=False))

    product_choice = int(input("Enter the number of the product you want to buy: "))
    selected_product = data[product_choice - 1]
    brand_index, model_index, price_index = header.index('Brand'), header.index('Model'), header.index('Price')
...
def checkout(purchase_history):
    ...
    print("\nInvoice:")
    print(tabulate(invoice_table, headers=['Product Name', 'Qty', 'Unit Price', 'Total Price'], tablefmt='grid'))
...
def main():
    ...
    while True:
        ...
        if choice == len(sheet_options) + 1:
            ...
            print(f"Added '{product_info['Product Name']}' to your cart. Total price: {product_info['Total Price']}")
        else:
